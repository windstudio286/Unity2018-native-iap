...
extern "C" int ParseDeviceGeneration(const char* model)
{
    ...
    else if (!strncmp(model, "iPhone12,8", 11))
        return deviceiPhoneSE2Gen;
    else if (!strncmp(model, "iPhone13,1", 11))
        return deviceiPhone12Mini;
    else if (!strncmp(model, "iPhone13,2", 11))
        return deviceiPhone12;
    else if (!strncmp(model, "iPhone13,3", 11))
        return deviceiPhone12Pro;
    else if (!strncmp(model, "iPhone13,4", 11))
        return deviceiPhone12ProMax;
    ...
    else if (!strncmp(model, "iPad8,", 6))
    {
        ...
        if (rev >= 1 && rev <= 4)
            ...
            return deviceiPadPro11Inch2Gen;
        else if (rev >= 11)
            return deviceiPadPro4Gen;
    }
    else if (!strncmp(model, "iPad11,", 7))
    {
        int rev = atoi(model + 7);
        if (rev == 1 || rev == 2)
            return deviceiPadMini5Gen;
        else if (rev == 3 || rev == 4)
            return deviceiPadAir3Gen;
        else if (rev == 6 || rev == 7)
            return deviceiPad8Gen;
    }
    else if (!strncmp(model, "iPad13,", 7))
    {
        int rev = atoi(model + 7);
        if (rev == 1 || rev == 2)
            return deviceiPadAir4Gen;
    }
